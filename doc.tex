...
Such methods were first introduce by Buffa and co-workers~\cite{Buffa2010iae,Buffa2011ias}
...
Navier-Stokes equation, projecting the result onto the pressure modes.
...
There are at least two ways to construct a combined basis. One way is to omit the discussion in
Section~\ref{sec:fieldsep}, and instead pick $z$ in \eqref{eqn:covmx} as joint velocity-pressure
inner product, for example
\begin{equation}
  \label{eqn:z_c}
  z_\textsc{v}\big((\hat{\bm u}_0, \hat{p}), (\hat{\bm v}_0, \hat{q})\big) =
  \left( \hat{\bm u}_0, \hat{\bm v}_0 \right)_{H^1(\hat{\Omega})}
  + \eta \left( \hat{p}, \hat{q} \right)_{L^2(\hat{\Omega})}
\end{equation}
where $\eta > 0$ can be chosen to appropriately scale the relative importance of velocity
...
not be linearly independent (however, linear dependence is not required). In the following we have
...
recovered pressures from Section~\ref{sec:conforming}. This may indicate that the problem setup is
not rich enough to capture the nonlinear pressure-velocity behaviour.
...
formulation was applied, with two choices for pressure recovery.
...
The two pressure recovery methods were also able to achieve similar performance for this
problem. Since the pressure reconstruction method (Section~\ref{sec:combined}) relies on a linearity
assumption (Remark~\ref{rem:linearity}) that is not valid in general, we must conclude that pressure
recovery (Section~\ref{sec:conforming}) is to be preferred in most cases.
