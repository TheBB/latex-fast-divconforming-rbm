...
\end{equation}
%
and the pullback of the linear forms $d_0$ and $d_1$ is defined analogously.
\begin{remark}
  \label{rem:lift}
  Note that the lift $\hat{\bm \ell}$ is constructed on the reference domain, and then transported
  via $\pi_{\bm \mu}^0$ to the parametrized domain. In this construction it is important to point
  out that $\pi_{\bm \mu}^0 \hat{\bm \ell}$ is assumed to actually be a valid lift, i.e.~it
  has the correct value on the Dirichlet boundaries, and that the reference lifts and
  transformations must be designed so that this assumption holds.
\end{remark}
...
$\hat{\bm \ell} = \sum_i \xi_i(\bm \mu) \hat{\bm \ell_i}$ where $\pi_{\bm\mu}^1 \bm \ell_i = \bm g_i$ on
...
\begin{equation}
  ...
  \qquad 1 \leq j \leq M.
\end{equation}
and they are orthonormal in the $z(\cdot,\cdot)$ inner product by construction.
...
\begin{equation}
  \frac{ \sum_{i=1}^N \| \varphi_i - \bm P_V \varphi \|_z^2 }{ \sum_{i=1}^N \| \varphi_i \|_z^2 }
...
\begin{equation}
  \sqrt{ \sum_{i=1}^N \| \varphi_i - \bm P_V \varphi_i \|_z^2 }
  \leq \epsilon \sqrt{ \sum_{i=1}^N \| \varphi_i \|_z^2 },
\end{equation}
...
\begin{equation}
  \sqrt{ \int_\mathcal{P} \| \varphi(\bm \mu) - \bm P_V \varphi(\bm \mu) \|_z^2 \dd\bm \mu }
  \leq \epsilon \sqrt{ \int_\mathcal{P} \| \varphi(\bm \mu) \|_z^2 \dd\bm \mu }.
\end{equation}
...
function
\begin{equation}
  z = z_u, \qquad a_u((\hat{\bm u}_0, \hat{p}), (\hat{\bm v}_0, \hat{q})) =
...
$a_z$ is globally only semidefinite, since the basis is restricted to only those
components on which $a_z$ \emph{is} definite, the basis functions remain
linearly independent.

A pressure basis may then be generated by exactly the same method using for
example
\begin{equation}
  z = z_p, \qquad z_p((\hat{\bm u}_0, \hat{p}), (\hat{\bm v}_0, \hat{q})) =
