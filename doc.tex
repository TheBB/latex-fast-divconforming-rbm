...
see, e.g.~\cite{Ballarin2015,Lovgren:2006sb}. However, it appears that the concept was not followed
...
Considering numerical approximations of~\eqref{ns-1}--\eqref{ns-4},
formulation~\eqref{eqn:velonlyform} has some practical benefits: in view of the
...
is often of interest, e.g. in a post-processing step to determine flow-induced loads by means
of extraction functions~\cite{MelboKvamsdal2003,Brummelen:2012fk}, impairing a solver
that does not provide pressure solutions in many applications.

The proposed solenoidal RB method forms an instance of the velocity-only formulation~\eqref{eqn:velonly}.
However, the problems mentioned above for conventional approximation methods are minor in the context of
RBM. First, the inherent solenoidality of the snapshots facilitates the construction of a divergence-free~\emph{reduced bases}. Each snapshot corresponds to the solution of a high-fidelity problem and, hence, the computational cost
of constructing the solenoidal basis is indeed significant. However, the computational cost of constructing
snapshots is inherent to RBM, and it is ignored because this expense is incurred in the offline stage.
Second, the pressure solution can be recovered \emph{a posteriori}, by solving \eqref{eqn:vvar-1} with a
...
possibly parameter-dependent, the corresponding homogeneous variational form
...
Let us next consider geometric parameters, by which we mean the possibility that
...
necessarily the case), such that there exists a suitably
smooth (and smoothly invertible) homeomorphism  $\chi_{\bm \mu} : \Omega(\bm \mu) \to\hat{\Omega}$,
which maps any of the parametrized domains to the reference domain. For the sake
...
\mu$. We introduce function spaces on the reference domain according to
%
\begin{align*}
...
\end{align*}
%
and two smooth maps ${\pi}_{\bm\mu}^0:\hat{U}_0\to{}U_0$ and ${\pi}_{\bm\mu}^1:\hat{P}_0\to{}P_0$,
noting that the $\bm\mu$\nobreakdash-dependence of $U_0$ and $P$ is accounted for in the maps $\pi^{0,1}_{\bm\mu}$.
For instance, $\bm{\pi}_{\bm\mu}:=({\pi}_{\bm\mu}^0,{\pi}_{\bm\mu}^1)$ can be defined as the canonical
transformation:
%
\begin{equation}
...
\end{equation}
%
However, alternate maps can be constructed. The pullback of the multi-linear forms $b,c,d$ (to the reference
configuration, under the map~$\bm{\pi}_{\bm\mu}$) is then defined by:
%
\begin{equation}
\begin{aligned}
({\bm\pi}^*_{\bm\mu}d)\big(\hat{\bm{u}}_0,\hat{\bm{w}};\bm{\mu}\big)
&=
d\big({\pi}_{\bm\mu}^0\hat{\bm{u}}_0,{\pi}_{\bm\mu}^0\hat{\bm{w}};\bm{\mu}\big)
\\
...
\begin{figure}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[red, thick, mark=*, mark options={solid}]
      table[x index={19}, y index={4}]{data/airfoil-results-piola-sups-no-block.csv};
      \addplot[red, thick, densely dashed, mark=o, mark options={solid}]
      table[x index={19}, y index={8}]{data/airfoil-results-piola-sups-no-block.csv};
