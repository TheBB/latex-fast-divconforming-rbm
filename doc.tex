...
In fact, the same expressions can be reached by setting $\bm B_\textsc{vp} = 0$ and
...
$b(\cdot,\cdot,\bm \mu)$ is not exact, the matrix $\bm B_\textsc{vp}$ from
...
that no significant degradation of stability is to be expected for
divergence-conforming reduced methods.

Table~\ref{tbl:subspaceangles} show the angles between the velocity and supremizer spaces. Since
linear independence between the two spaces are not enforced \emph{a priori}, these angles are
relevant to the question whether the sum velocity space may possibly be degenerate. This shows that
for all bases considered in this work, the two spaces are suitably linearly
independent. Nevertheless, the divergence-conforming bases show remarkable near-orthogonality for
larger $M$ that the Taylor-Hood method does not match.
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{crrrr}
      ...
      \hline
      $\sharp$ DoFs ($M$) & \multicolumn{1}{c}{Unstabilized} & \multicolumn{1}{c}{Stabilized} & \multicolumn{1}{c}{Stabilized} \\
      \hline $10$ & $\SI{8.06e-6}{}$ & $\SI{3.93e-1}{}$ & $\SI{4.11e-1}{}$ \\
      $20$ & $\SI{5.68e-6}{}$ & $\SI{2.54e-1}{}$ & $\SI{3.37e-1}{}$ \\
      $30$ & $\SI{1.03e-6}{}$ & $\SI{3.15e-1}{}$ & $\SI{3.13e-1}{}$ \\
      $40$ & $\SI{2.65e-8}{}$ & $\SI{2.67e-1}{}$ & $\SI{2.90e-1}{}$ \\
      $50$ & $\SI{1.77e-9}{}$ & $\SI{2.75e-1}{}$ & $\SI{2.72e-1}{}$ \\
      \hline
    \end{tabular}
    \egroup
  \end{center}
...
\begin{table}
  ...
  \end{center}
  \caption{
    Angles between the velocity and supremizer spaces for the two methods for various basis sizes.
  }
...
isogeometric high-fidelity method. Supremizers used to stabilize the former \cite{Ballarin2015ssp}
and to provide pressure solutions for the latter
...
terms ($\sim 12n$ versus $\sim 4n$). In spite of this, the conforming method
yields faster online stages. We aim to see in future work that automatic ``black box''
...
alleviates the additional work needed to produce such affine representations.
